...
export const BookingDetails: React.FC = () => {
  ...
  const downloadBooking = () => {
    ...
    const drawApprovalSection = () => {
      ...
      doc.setFontSize(labelFontSize);
      doc.setTextColor(100, 116, 139);
      doc.text('ISSUED BY', left + innerPaddingX, y + labelTop);
      ...
      doc.setFontSize(9 * layoutScale);
      doc.setTextColor(71, 85, 105);
      doc.text(`Generated by: ${generatedBy}`, left + innerPaddingX, y + 55 * layoutScale);
      doc.text(`Generated on: ${generatedOn}`, left + innerPaddingX, y + 70 * layoutScale);

      doc.setDrawColor(148, 163, 184);
      doc.setLineWidth(0.8);
      const signatureLineY = y + sectionHeight - 34 * layoutScale;
      doc.line(left + innerPaddingX, signatureLineY, left + signatureWidth - innerPaddingX, signatureLineY);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8 * layoutScale);
      doc.setTextColor(100, 116, 139);
      doc.text('Authorised signature', left + innerPaddingX, signatureLineY + 12 * layoutScale);
