...
export const BookingDetails: React.FC = () => {
  ...
  const downloadBooking = () => {
    ...
    const generatedBy = user?.full_name?.trim() || user?.email?.trim() || 'System Administrator';
    ...
    const drawApprovalSection = () => {
      const sectionHeight = 128 * layoutScale;
      const columnGap = 16 * layoutScale;
      const signatureWidth = (contentWidth * 0.58) - columnGap / 2;
      const stampWidth = contentWidth - signatureWidth - columnGap;

      ensureSpace(sectionHeight + rowGap);

      doc.setDrawColor(203, 213, 225);
      doc.setFillColor(255, 255, 255);
      doc.roundedRect(left, y, signatureWidth, sectionHeight, boxRadius, boxRadius, 'FD');
      doc.roundedRect(left + signatureWidth + columnGap, y, stampWidth, sectionHeight, boxRadius, boxRadius, 'FD');

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(labelFontSize);
      doc.setTextColor(100, 116, 139);
      doc.text('GENERATED BY', left + innerPaddingX, y + labelTop);
      doc.text('OFFICIAL STAMP', left + signatureWidth + columnGap + innerPaddingX, y + labelTop);

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(valueFontSize);
      doc.setTextColor(15, 23, 42);
      doc.text(generatedBy, left + innerPaddingX, y + 36 * layoutScale);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9 * layoutScale);
      doc.setTextColor(71, 85, 105);
      doc.text(`Generated on: ${generatedOn}`, left + innerPaddingX, y + 55 * layoutScale);
      doc.text('Approved By:', left + innerPaddingX, y + 74 * layoutScale);

      const approvedByLineY = y + 77 * layoutScale;
      doc.setDrawColor(148, 163, 184);
      doc.setLineWidth(0.8);
      doc.line(left + 88 * layoutScale, approvedByLineY, left + signatureWidth - innerPaddingX, approvedByLineY);

      const signatureLineY = y + sectionHeight - 30 * layoutScale;
      doc.line(left + innerPaddingX, signatureLineY, left + signatureWidth - innerPaddingX, signatureLineY);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8 * layoutScale);
      doc.setTextColor(100, 116, 139);
      doc.text('Signature', left + innerPaddingX, signatureLineY + 12 * layoutScale);

      const stampInset = 16 * layoutScale;
      const stampBoxTop = y + 28 * layoutScale;
      const stampBoxHeight = sectionHeight - 48 * layoutScale;
      doc.setDrawColor(203, 213, 225);
      doc.setLineWidth(0.8);
      doc.roundedRect(
        left + signatureWidth + columnGap + stampInset,
        stampBoxTop,
        stampWidth - stampInset * 2,
        stampBoxHeight,
        10 * layoutScale,
        10 * layoutScale,
        'D'
      ...
      const stampLabel = 'Stamp here';
      const stampLabelWidth = doc.getTextWidth(stampLabel);
      doc.text(
        stampLabel,
        left + signatureWidth + columnGap + (stampWidth - stampLabelWidth) / 2,
        stampBoxTop + stampBoxHeight / 2
      );
    ...
    drawTwoColumnRow([
      ...
      { label: 'Booking Reference', value: booking.reference_no },
    ], 58);
    drawApprovalSection();
