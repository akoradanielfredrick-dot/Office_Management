...
export const ExcursionForm: React.FC = () => {
  ...
  const onSubmit = async (data: ExcursionValues) => {
    ...
    try {
      if (isEditMode && id) {
        await api.put(`/operations/excursions/${id}/`, data);
      } else {
        await api.post('/operations/excursions/', data);
      }
      navigate('/excursions');
    } catch (error) {
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-500">
              Save excursion records directly to the backend excursion service so the frontend stays aligned with the operations catalog.
            </p>
          </div>
        </div>
